...
<div class="card">
	<div class="card-header">
		<form id="buscarempleado" action="busqueda" onKeypress="if(event.keyCode == 13) event.returnValue = false;">
			<!-- {{ csrf_field() }} -->
			<div class="row">
				<div class="col mt-3">
				...
				<tr class="active" title="Has Click Aquì para Ver" style="cursor: pointer" href="#{{$cotizacion->id}}">
					...
					<td>
						...
						<a class="btn btn-success btn-sm"
							href="{{ url('clientes/'.$cliente->id.'/'.$cotizacion->id.'/cotizacion/show') }}" /><i
							class="fa fa-eye" aria-hidden="true"></i>
						<strong>Ver</strong> </a>
						<a class="btn btn-warning btn-sm"
							href="{{ url('clientes/'.$cliente->id.'/'.$cotizacion->id.'/edit') }}" /><i
							class="fas fa-edit" aria-hidden="true"></i>
						<strong>Editar</strong></a>
						@endisset
						@isset ($prospecto)
						<a class="btn btn-success btn-sm"
							href="{{ url('prospectos/'.$prospecto->id.'/'.$cotizacion->id.'/show') }}" /><i
							class="fa fa-eye" aria-hidden="true"></i>
						<strong>Ver</strong></a>
						<a class="btn btn-warning btn-sm"
							href="{{ url('prospectos/'.$prospecto->id.'/'.$cotizacion->id.'/edit') }}" /><i
							class="fas fa-edit" aria-hidden="true"></i>
						<strong>Editar</strong></a>
						@endisset
						<a class="btn btn-primary btn-sm" href="{{route('cotizaciones.download', ['cotizacion'=>$cotizacion->id])}}"><i class="fas fa-file" aria-hidden="true"></i>
